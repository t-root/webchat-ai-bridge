...
**Kết nối Claude, ChatGPT, Gemini, DeepSeek, Grok, Copilot với terminal / script qua AI API OpenAI-compatible.**
...
| **Model `auto`** | Tự chọn ngẫu nhiên một model có sẵn khi gửi `"model": "auto"` |
| **Tự reload khi timeout** | Hết thời gian chờ → reload trang model → gửi lại prompt từ đầu |
| **API OpenAI-compatible** | `POST /v1/chat/completions`, `GET /v1/models`, streaming SSE cơ bản |
| **API key** | Xác thực Bearer token (mặc định `trungdeptrai`) — tương thích client OpenAI |
...
| **Điền prompt** | Editor rich-text luôn paste (Ctrl+V); `textarea`/`input` dùng `fill()` |
...
| **Hiện / ẩn trình duyệt** | `python server.py` hiện cửa sổ; `python server.py --hidden` chạy headless |
...
```bash
git clone https://github.com/t-root/webchat-ai-bridge.git
cd webchat-ai-bridge
```
...
Ẩn trình duyệt (headless — không mở cửa sổ):

```bash
python server.py --hidden
```

| Lệnh | Trình duyệt |
|------|-------------|
| `python server.py` | Hiện cửa sổ Chrome (mặc định) |
| `python server.py --hidden` | Ẩn — chạy nền |
...
curl -X POST http://127.0.0.1:5000/v1/chat/completions \
  ...
  -H "Authorization: Bearer trungdeptrai" \
  -d '{
    "model": "claude",
    "messages": [
...
**API key** (mặc định trong `client.api_key`):

| Cách gửi | Ví dụ |
|----------|--------|
| `Authorization: Bearer …` | `Authorization: Bearer trungdeptrai` |
| Header `api-key` | `api-key: trungdeptrai` |

Endpoint `/health` không yêu cầu key. `/v1/models` và `/v1/chat/completions` **bắt buộc** có key hợp lệ — sai key trả `401` theo format lỗi OpenAI.

**Model ID** (theo `model` trong config):

| `model` | Nền tảng |
|---------|----------|
| `auto` | Tự chọn ngẫu nhiên một model có sẵn |
| `claude` | Claude |
| `gpt` | ChatGPT |
| `gemini` | Gemini |
| `deepseek` | DeepSeek |
...
Bridge chọn ngẫu nhiên một model trong `ai_models_config.json` (Claude, ChatGPT, Gemini, …). Nếu timeout hoặc lỗi, tự reload trang và thử lại (xem [Retry khi timeout](#retry-khi-timeout)).
...
| `API_KEY` | `client.api_key` | API key cho `/v1/*` (mặc định `trungdeptrai`) |
...
Khi model không trả kết quả trong thời gian chờ (`max_wait_ms`) hoặc response rỗng, bridge tự **reload trang** (xóa UI đang kẹt / thanh generating) rồi **gửi lại prompt từ đầu** với cùng nội dung.

Cấu hình trong section `retry`:

| Key | Mặc định | Mô tả |
|-----|----------|--------|
| `max_attempts` | `2` | Số lần thử (1 lần đầu + reload retry) |
| `reload_on_failure` | `true` | Reload trang trước khi thử lại |

Ví dụ log khi retry:

```
[Playwright] Gemini timeout on attempt 1/2 — will reload and retry
[Playwright] Reloading Gemini page (retry after timeout)...
```
...
- `textarea`/`input` → `fill()` (điền thẳng, mọi độ dài)
- `contenteditable` → copy vào clipboard rồi **paste (Ctrl+V)**
...
- Editor rich-text: bridge luôn paste prompt; nếu vẫn lỗi, thử đăng nhập lại trang AI
...
- Bridge tự reload trang và thử lại theo `retry.max_attempts` (mặc định 2 lần)
- Tăng `response_settings.max_wait_ms` nếu model trả lời chậm
- Kiểm tra log `[Playwright] Reloading ... page (retry after timeout)`
...
pip install -r requirements.txt
python -m playwright install chromium
python server.py              # hiện trình duyệt
# python server.py --hidden   # ẩn trình duyệt (headless)
# Terminal khác:
python client_test.py
```

**Nhớ:** mỗi AI = `url` + **4 selector** (`input`, `send_button`, `stop_button`, `message_frame`). Model `auto` chọn ngẫu nhiên; timeout tự reload trang theo `retry` trong JSON.
...
| Thay đổi | Mô tả |
|----------|--------|
| **Model `auto`** | `"model": "auto"` — bridge chọn ngẫu nhiên model có sẵn; hỗ trợ alias `random` |
| **Retry + reload** | Timeout hoặc lỗi → reload trang model → gửi lại prompt; cấu hình qua `retry` trong JSON |
| **Hiện / ẩn browser** | `python server.py` (hiện) · `python server.py --hidden` (headless) |
